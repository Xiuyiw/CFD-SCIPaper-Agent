...
class PlotStyle(PublicationModel):
    """Local plotting dimensions in mm and text sizes in pt."""

    model_config = ConfigDict(extra="forbid", allow_inf_nan=False)
    width_mm: float = Field(default=165.1, gt=0)
    height_mm: float = Field(default=106.68, gt=0)
    dpi: int = Field(default=300, gt=0)
    font_family: str = Field(default="DejaVu Serif", min_length=1)
    font_pt: float = Field(default=10, gt=0)
    axis_label_pt: float = Field(default=11, gt=0)
    title_pt: float = Field(default=11, gt=0)
    tick_pt: float = Field(default=9, gt=0)
    legend_pt: float = Field(default=9, gt=0)
    annotation_pt: float = Field(default=8.5, gt=0)
...
def _script_text(figure_id: str, style: PlotStyle | None = None) -> str:
    ...
    return textwrap.dedent(
        ...
        PLOT_STYLE = {style.model_dump()!r}
        FIGURE_SIZE = (PLOT_STYLE["width_mm"] / 25.4, PLOT_STYLE["height_mm"] / 25.4)
        DPI = PLOT_STYLE["dpi"]
        MARKER_STYLE = "o"
        MARKER_SIZE = 38
        LINE_STYLE = "-"
        ...
        def create_figure(rows: list[dict[str, str]]):
            ...
            plt.rcParams.update({{
                ...
                "font.size": PLOT_STYLE["font_pt"],
                "axes.labelsize": PLOT_STYLE["axis_label_pt"],
                "axes.titlesize": PLOT_STYLE["title_pt"],
                "xtick.labelsize": PLOT_STYLE["tick_pt"],
                "ytick.labelsize": PLOT_STYLE["tick_pt"],
                "legend.fontsize": PLOT_STYLE["legend_pt"],
                "svg.fonttype": "none",
                "pdf.fonttype": 42,
            ...
            annotations = [
                ax.annotate(
                    ...
                    ha="center",
                    va="bottom",
                    fontsize=PLOT_STYLE["annotation_pt"],
                    clip_on=False,
                )
...
def _write_script(path: Path, figure_id: str, style: PlotStyle | None = None) -> None:
    path.write_text(_script_text(figure_id, style), encoding="utf-8")


def _protect_local_script(script_path: Path, delivery_path: Path, figure_id: str) -> None:
    if not script_path.exists():
        return
    try:
        previous = FigureDeliveryManifest.model_validate_json(delivery_path.read_bytes())
        unchanged = previous.figure_id == figure_id and previous.files.get(
            script_path.name
        ) == _sha256(script_path)
    ...
    if not unchanged:
        raise FigureRenderError(
            f"Refusing template overwrite: local plotting script differs from its delivery "
            f"record or has no usable baseline: {script_path}. "
            f"Keep this script as the source for targeted edits and re-export with "
            f'python "{script_path}"; do not replace it with the generated template. '
            "Re-export alone does not refresh delivery validation."
        )
...
def _bundle_paths(output: Path, figure_id: str) -> dict[str, Path]:
    return {
        ...
        f"{figure_id}.pdf": output / f"{figure_id}.pdf",
        f"{figure_id}.tiff": output / f"{figure_id}.tiff",
...
def _raster_exports_match(png_path: Path, tiff_path: Path) -> bool:
    from PIL import Image

    try:
        with Image.open(png_path) as png, Image.open(tiff_path) as tiff:
            png.load()
            tiff.load()
            return (
                png.format == "PNG"
                and tiff.format == "TIFF"
                and png.size == tiff.size
                and png.convert("RGB").getextrema() != ((255, 255),) * 3
                and tiff.convert("RGB").getextrema() != ((255, 255),) * 3
            )
    except (OSError, ValueError):
        return False
...
def _rendered_sizing(script_path: Path, png_path: Path) -> tuple[float, float]:
    from matplotlib.text import Text
    from PIL import Image

    module = _load_plot_module(script_path)
    figure, _, _, _ = module.create_figure(module.load_source_data())
    try:
        figure.canvas.draw()
        minimum_pt = min(
            text.get_fontsize()
        ...
        with Image.open(png_path) as raster:
            width_mm = raster.width * 25.4 / raster.info["dpi"][0]
        return width_mm, minimum_pt
    finally:
...
def _layout_is_inside_figure(script_path: Path) -> bool:
    try:
        ...
        # Locators can return ticks outside the view interval; Matplotlib does not
        # draw those labels, even though their Text artists report visible=True.
        tick_artists = [
            label
...
def _build_visual_qa(
    *,
    ...
    try:
        module = _load_plot_module(script_path)
        expected_pixels = tuple(size * module.DPI for size in module.FIGURE_SIZE)
        dimensions_pass = dimensions is not None and all(
            abs(actual - expected) <= 1
            for actual, expected in zip(dimensions, expected_pixels, strict=True)
        )
    except Exception:
    ...
    checks = [
        ...
        _check(
            "png-dimensions",
            dimensions_pass,
            "PNG reopens at the configured physical canvas and dpi",
            "PNG is invalid or differs from the configured physical canvas and dpi",
        ),
        _check(
            "raster-exports",
            _raster_exports_match(png_path, tiff_path),
            "PNG and TIFF decode with matching canvas dimensions and nonblank content",
            "PNG or TIFF cannot decode, has a different canvas, or is blank",
        ),
        _check(
...
def _bundle_components(
    *,
    ...
    artifacts = (
        ...
        FigureArtifact(path=paths[f"{candidate.figure_id}.pdf"], role="editable"),
        FigureArtifact(path=paths[f"{candidate.figure_id}.tiff"], role="preview"),
    )
    ...
    qa_results = (
        ...
        _build_visual_qa(
            ...
            svg_path=svg_path,
            png_path=png_path,
            tiff_path=paths[f"{candidate.figure_id}.tiff"],
            script_path=script_path,
            source_binding=source_binding,
            artifact_bindings=artifact_bindings,
            manifest=manifest,
...
def build_figure_delivery(
    *,
    ...
    svg_path = paths[f"{candidate.figure_id}.svg"]
    png_path = paths[f"{candidate.figure_id}.png"]
    _protect_local_script(script_path, paths["delivery.json"], candidate.figure_id)
    output.mkdir(parents=True, exist_ok=True)
    _write_source_data(source_path, _expected_source_rows(candidate, analysis))
    _write_script(script_path, candidate.figure_id, style)
    paths["caption.txt"].write_text(_caption_text(candidate), encoding="utf-8")
    _run_script(script_path)
    source_width_mm, minimum_source_font_pt = _rendered_sizing(script_path, png_path)
    ...
    delivery_manifest = FigureDeliveryManifest(
        ...
        minimum_source_font_pt=minimum_source_font_pt,
    )
    ...
    return FigureDelivery(
        ...
        svg_path=svg_path,
        png_path=png_path,
        pdf_path=paths[f"{candidate.figure_id}.pdf"],
        tiff_path=paths[f"{candidate.figure_id}.tiff"],
